...
End-to-end tests (`tests/e2e`) drive the real `*mcp.Server` through the MCP `tools/call` boundary against a live Postgres selected by `SQLDB_MCP_TEST_POSTGRES_URL` (a libpq keyword/value DSN). The test harness connects an SDK client to `server.New` over an in-memory transport, so tool registration, registry resolution, and the handler→dialect contract are all exercised. A test helper auto-loads a repo-root `.env`, so `go test ./...` works once `.env` contains the variable; an explicit export or CI secret still wins. When unset the suite skips:
