...
The `explain_query` tool: runs `EXPLAIN` for a query and returns the plan,
optionally with `ANALYZE` to execute it while planning. Respects read-only mode
(ANALYZE of a write fails when restricted) and is bounded by the same per-query
timeout as `execute_query`.
